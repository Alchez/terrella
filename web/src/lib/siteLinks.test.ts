...
// a convenience. It sits in the top-left chrome row, but it is still MapLibre's own control.
describe("the on-map credit", () => {
  ...
  it("points at the page that carries the verbatim licence notices", () => {
    ...
    expect(CREDITS).toContain('href="/about/"');
  });
  ...
  it("keeps an accessible name, the credit now being a glyph rather than a word", () => {
    ...
    // just discoverability: a link whose only content is a decorative glyph has no accessible
    ...
    expect(CREDITS).toContain('aria-hidden="true"'); // the glyph itself, so the name is not doubled
  });
});
