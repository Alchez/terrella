...
/** The public source repository, linked from the gallery masthead, the globe's chrome and the footer. */
export const REPO_URL = "https://github.com/Alchez/terrella";

/** GitHub's octicon, drawn by all three of those. Each site sizes and colours its own `<svg>`, so
 *  only the geometry is here: an Astro component would put the element in its own style scope and
 *  take `.head-source svg` and `.globe-source svg` with it. */
