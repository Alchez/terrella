...
### Both surfaces (`palette.py` and `scene_build.RIG`): the hero batch plus a whole-planet re-render

The globe's tiles render through the same rig as the heroes, so a `RIG` field reaches both.
...
- Data, not paint: a hero's `snowmask.png` is the tiles' own white, NSIDC-0791 persistence and RGI 7.0 glaciers folded by the same producers on the country's grid. The shader branch keys on the snow stage's declaration, so with no mask declared the graph is identical to pre-snow.
...
- **Fix:** a **60 m box-mean lowpass** of the warped heightfield, auto-engaging only above 5× upsample, so it selects the microstates by geometry and never softens larger countries. It is a formula rather than a hand list, striping severity being a clean function of upsample factor, and it caught nauru where a hand list missed it.
