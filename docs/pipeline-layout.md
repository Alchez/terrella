...
**A seam splits when one half outgrows its package and the other does not.** `render_files.py` names the images a render directory holds and is read well beyond the rig, so it is a top-level law; `render/render_seam.py` records which stage wrote which of them and is read only by the rig, so it stays. `planet_seam.py` is not split that way because both of its halves have that same wide readership, which is the test rather than the shape.
