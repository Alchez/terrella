...
Every hero render shares one *look*: the same sun, the same colors, the same 15× vertical exaggeration. The colours and the sun altitude are imported from `palette.py`; the exaggeration is `Body.baked_exaggeration`, which is 15× because heroes are Earth's and Earth's field is pinned to the authored constant. What changes from country to country is only *geometry*: where the camera looks, and how numbers convert between the real world and Blender's little stage. This page explains each conversion in plain English, with the formulas alongside.
...
- **Displacement scale**: the heightfield stores real meters. Multiplying by scale must turn meters into Blender units *and* apply the body's exaggeration: `scale = exaggeration ÷ (extent_w / 2)`, which is `15 ÷ (extent_w / 2)` for every hero. The value used is recorded in the frame.json beside the scale it produced, so a pinned frame can say which one it was built at. This is why the number is different per country while the mountains' *relative* drama is identical: a small frame means each Blender unit is fewer meters, so each meter of rock is a bigger fraction of a unit. Copying India's value onto Switzerland would exaggerate ~100×, turning mountains into walls.
