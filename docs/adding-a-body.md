...
- **`BodyDescriptor.meshExaggeration` is nullable**, the one deliberate hole in the scheme, so a body that answers `null` gets a flat globe by default and reaches displacement only through `?terrain=N`. The scale has to be ratified by eye, and shipping displacement nobody has looked at is the worse failure. Its sibling `bakedExaggeration` is not nullable and is not this number: it mirrors `Body.baked_exaggeration`, already in the pixels, where this one runs a live mesh.
- **An optional field or an index signature on `BodyDescriptor` turns the table above into silent inheritance**, which `bodies.ts` refuses in its own header. The compile error is the mechanism; remove it and the diff that adds a field still looks complete.
- **A registry fallback to Earth does the same in Python.** `palette.look_for`'s docstring calls it the tempting kindness: a body nobody registered then renders a plausible pyramid in Earth's colours with every gate passing. A named raise listing the known bodies is the shape to keep.
- **Nothing resolves a hero subject but Earth.** The hero lane renders one country, and `frame/country_config.py` derives every frame from Natural Earth bounds with `bodies.EARTH` hardcoded, which is why `batch.py` takes no `--body` at all. A body that answers nothing here has no heroes, and no gate says so.
- **The machinery is inherited and only the subject is not.** `render_prep.scene_numbers` is the single home for the framing maths and already serves three windows, a country, a tile block and a polar cap, all handing one `scene_build.py` a directory that `render_seam.py` declares. What a third body would bring is a registry yielding a slug and bounds; whether it wants one is a decision, and Mars declined it and ships the globe alone.
