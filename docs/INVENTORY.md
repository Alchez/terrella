...
| `borders/` | 21 MB | `countries.geojson` (the vector cut's input), `boundary_lines.geojson` and `tectonic_boundaries.geojson`, served at `/borders/` | Keep (tiny); regenerable from `naturalearth/` |
...
- `boundary_lines` and `tectonic_boundaries` each load only after the visitor opts into their overlay: async, first paint never waits.
