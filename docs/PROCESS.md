...
| Site deploy | `pnpm run deploy` in `web` | preflight ~6 s · build ~0.5 s · upload **13 s** | Never `wrangler deploy` alone: that ships a same-origin build whose hero pages 404. The tile Worker deploys first, always: the pair is safe in exactly one order, because a new Worker serves an old site and an old Worker serves nothing a new site asks for. Skipping it 404s every tile. Verify against the live host, never the repo |
| Deploy preflight | `pnpm run check:deploy-sync` | **~6 s** (two listings, 3,498 objects in the asset bucket) | Advertised-but-absent is fatal, and so is a download file or the bundle at another size than recorded; present-but-unreferenced warns. Needs `R2_ENDPOINT` and the `r2` profile |
...
| R2 upload, heroes + overlays | `aws … s3 sync … --exclude "*.aux.xml" --exclude "*_recipe.json" --exclude "*.png"` | **~2 min** (1,622 files, 2.13 GB) | The first two excludes are mandatory: GDAL PAM sidecars are the bulk, and the recipe is internal freshness state that must not be published. The third leaves the PNG copies to `upload_downloads.ts`, which also re-sends each changed full-size WebP the sync carries, with the header that makes a link save rather than open. GeoJSON needs `--content-type application/json` or the edge will not compress it |
| Download files, uploaded | `upload_downloads.ts --upload` | **29 min** (407 files, 15.80 GB) | One `aws s3 cp` per file in sequence, so about half the archive row's rate. Detach it |
| Download files, what a visitor is served | `upload_downloads.ts --verify` | **6.3 min** before the upload (407 files, one byte each, in sequence) | Reads through the public hosts, so it sees the edge's copy rather than R2's. One byte of an uncached file makes the edge fetch all of it, so the first verify after a purge fills the cache with the whole set and is untimed |
| Download files, the credit inside them | `downloads.py stamp` | **24 s** (203 × 2 files); a second pass **0.5 s** | Serial. Each full-size WebP is stamped in place and each master copied beside it, 11.4 GB written, peak 0.45 GB with a master held whole. A current file is left alone, so a second pass writes nothing |
| Country maps bundle | `downloads.py bundle` | **3.0 s** with its inputs out of the page cache, **1.9 s** warm (203 WebPs, 1.79 GB written) | Serial and streamed, peak 0.11 GB. It always rewrites the zip, and a rebuild of unchanged files is byte-identical |
...
| Spotlight overlays, every rung | `gen_spotlight.py --force` | **65 min** serial (203 slugs, 1,243 files, the native rung included); 7 s (Chile) to 36 s (Australia) a slug | The native rung peaks at 9.08 GB on the squarest frame, so two large slugs at once exceed the heavy-job cap. Serial is the default for that reason |
...
**Measure a dedupe figure rather than believing one written here.** A count in prose has no reader that can go red, and the two above replace figures that had drifted to 1.4% and ~5% against a true 0.17% and 0.007%. `tools/pmtiles show <archive>` prints `addressed tiles count` and `tile contents count`, and the difference between them is the whole of it. Where dedupe is real is the vector pyramids, at 26,000 of Earth's 40,333 addresses and 10,000 of Mars's 21,354.
