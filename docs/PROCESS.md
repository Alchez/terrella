...
| 4 | `tile/block_render.py`, the raytraced producer: every block through Cycles, one at a time | **11:41:33**, 1024 blocks at 1.46 blk/min, 0 failures. Per block 23.8 s min, 34.5 s median, 76.8 s p95, 194.3 s max | **2:49:57**, 256 blocks, 0 failures | ~0 s. A block re-renders when the ground under its own plane window moved, so a regional change costs its own blocks, while a recipe change still costs the planet | `planet_rgb.tif` 30 GB | per-block digest + `raytrace_params.json` |
...
| Site deploy | `pnpm run deploy` in `web` | preflight ~6 s · build ~0.5 s · upload **13 s** | Never `wrangler deploy` alone: that ships a same-origin build whose 204 hero pages 404. The tile Worker deploys first, always: the pair is safe in exactly one order, because a new Worker serves an old site and an old Worker serves nothing a new site asks for. Skipping it 404s every tile. Verify against the live host, never the repo |
