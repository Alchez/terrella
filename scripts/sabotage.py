...
def _earth_lake_depth''',
    ...
    Sabotage(
        ...
        path='web/DEPLOY.md',
        needle='the layer segment is `relief`, `terrain` or `vector`',
        replacement='the layer segment is `relief`, `terrain` or `countries`',
    ...
    Sabotage(
        ...
        # The archives section lists this source twice and the source map by key once more after the
        # cards, so the About card is the third match of four. Mutating the first would blank an
        # archive's list and leave this guard, which reads the cards, green.
        expected_matches=4,
        mutate_match=3,
    ),
    ...
    Sabotage(
        ...
        path='web/src/lib/tileAddress.ts',
        needle='      objectKey: "earth/terrain-v3.pmtiles",',
        replacement='      objectKey: "earth/planet-v5.pmtiles",',
    ...
    Sabotage(
        ...
        path='web/src/lib/tileAddress.ts',
        needle='      objectKey: "earth/countries-v3.pmtiles",',
        replacement='      objectKey: "earth/planet-v5.pmtiles",',
        guard='never puts two pyramids in one archive',
    ),
    ...
    Sabotage(
        ...
        needle='  "vars": {',
        replacement='  "vars": {\n    "RELIEF_OBJECT": "earth/planet-v3.pmtiles",',
    ...
    Sabotage(
        ...
        path='pipeline/attribution.py',
        needle='        heroes=("snow_persistence", "rgi", "naturalearth", "gwl", "worldcover", "globathy"),',
        replacement='        heroes=("snow_persistence", "rgi", "naturalearth", "gwl", "worldcover"),',
    ...
    Sabotage(
        ...
        path='pipeline/attribution.py',
        needle='        heroes=("snow_persistence", "rgi", "naturalearth", "gwl", "worldcover", "globathy"),',
        replacement='        heroes=("snow_persistence", "rgi", "gwl", "worldcover", "globathy"),',
    ...
    Sabotage(
        ...
        path='pipeline/attribution.py',
        needle='        heroes=("snow_persistence", "rgi", "naturalearth", "gwl", "worldcover", "globathy"),',
        replacement='        heroes=("snow_persistence", "rgi", "naturalearth", "worldcover", "globathy"),',
        guard='test_the_hero_credit_is_what_the_lane_reads',
    ),
    Sabotage(
        suite='python',
        ...
        path='pipeline/attribution.py',
        needle='            "salt_flats": ("naturalearth", "gwl", "worldcover", "snow_persistence"),',
        replacement='            "salt_flats": ("naturalearth", "worldcover", "snow_persistence"),',
        guard='test_earth_relief_carries_every_licence_required_notice',
    ),
    # The tempting tidy-up: every composed credit already carries WorldCover through the heightfield,
    # so dropping it from the salt looks free, and leaves the salt uncredited the day the DEM's gaps
    # stop needing it.
    Sabotage(
        suite='python',
        label='the salt flats stop naming WorldCover though it decides which saline ground they keep',
        path='pipeline/attribution.py',
        needle='            "salt_flats": ("naturalearth", "gwl", "worldcover", "snow_persistence"),',
        replacement='            "salt_flats": ("naturalearth", "gwl", "snow_persistence"),',
        guard='test_the_salt_names_worldcover_which_decides_its_saline_ground',
    ),
    Sabotage(
        suite='web',
    ...
    Sabotage(
        ...
        path='web/src/lib/countryPages.ts',
        needle='  const focus = country.hasSpotlight ? CREDITS.heroes.earth.focus : [];\n',
        replacement='  const focus = CREDITS.heroes.earth.focus;\n',
        guard="add the Focus layer's only where the page draws one",
    ),
    ...
    Sabotage(
        ...
        guard='test_both_credit_fields_are_the_credit_it_is_given',
    ),
    Sabotage(
        suite='python',
    ...
    Sabotage(
        ...
        path='pipeline/compose/downloads.py',
        needle='            archive.writestr(zip_entry(README, len(text)), text)\n',
        replacement='            archive.writestr(README, text)\n',
    ...
    Sabotage(
        ...
        path='pipeline/compose/downloads.py',
        needle='        if webp_packet(webp) != xmp_packet(titles[master.stem], credit_for(master)):\n',
        replacement='        if False:\n',
    ...
    # A hero's credit is what its render read. Each case below is a way of reading today's lane
    # instead, or of letting an output an earlier version wrote vouch for today's sources.
    Sabotage(
        suite='python',
        label="a hero file is credited with what today's lane reads, not what its render read",
        path='pipeline/compose/downloads.py',
        needle='    return attribution.compose(attribution.hero_record(master))\n',
        replacement='    return attribution.compose(attribution.hero_keys(bodies.EARTH))\n',
        guard='test_the_credit_is_the_one_recorded_for_its_render',
    ),
    Sabotage(
        suite='python',
        label="a hero's record is written from the lane rather than from what its stages declared",
        path='pipeline/batch.py',
        needle='                sources = attribution.declared_hero_sources(country_render_dir(slug))\n',
        replacement='                sources = (*attribution.CREDITS["earth"].heightfield,\n'
                    '                           *attribution.CREDITS["earth"].heroes)\n',
        guard='test_the_hero_is_recorded_as_built_from_what_its_stages_declared',
    ),
    Sabotage(
        suite='python',
        label="a stage that never declared its sources is read as having read none",
        path='pipeline/attribution.py',
        needle='        read = render_seam.stage_sources(render_dir, stage)\n',
        replacement='        read = render_seam.stage_sources(render_dir, stage) or []\n',
        guard='test_a_stage_that_never_declared_its_sources_is_named_rather_than_guessed',
    ),
    Sabotage(
        suite='python',
        label="a hero's record is believed after its master is rendered again",
        path='pipeline/attribution.py',
        needle='    if (record["bytes"], record["mtime_ns"]) != (rendered.st_size, rendered.st_mtime_ns):\n',
        replacement='    if False:\n',
        guard='test_a_master_rendered_again_after_its_record_is_refused',
    ),
    Sabotage(
        suite='python',
        label="the lake stage vouches for its sources when it skips an existing raster",
        path='pipeline/render/lake_mask.py',
        needle='        render_seam.declare(render_dir, render_seam.LAKE, [render_files.LAKEDEPTH])\n'
               '        return\n',
        replacement='        declare_written(render_dir)\n'
                    '        return\n',
        guard='test_a_skipped_depth_raster_vouches_for_no_sources',
    ),
    Sabotage(
        suite='python',
        label="the prep vouches for its sources when it skips an existing heightfield",
        path='pipeline/render/render_prep.py',
        needle='    if heightfield_written:\n',
        replacement='    if True:\n',
        guard='test_a_skipped_heightfield_vouches_for_no_sources',
    ),
    Sabotage(
        suite='python',
        label="the snow stage writes its masks and never says what it read",
        path='pipeline/render/snow_mask.py',
        needle='    render_seam.declare_sources(render_dir, render_seam.SNOW,\n'
               '                                attribution.hero_stage_sources(body, render_seam.SNOW))\n',
        replacement='',
        guard='test_the_stage_declares_what_it_read_when_it_writes',
    ),
    Sabotage(
        suite='python',
        label="the bundle states one credit for heroes rendered from different sources",
        path='pipeline/compose/downloads.py',
        needle='    if len(distinct) > 1:\n',
        replacement='    if False:\n',
        guard='test_images_from_renders_of_different_sources_are_refused_by_name',
    ),
    Sabotage(
        suite='python',
        label="the stamp command stops at the first master with no record",
        path='pipeline/compose/downloads.py',
        needle='        except attribution.HeroRecordError as unstated:\n'
               '            refused.append(str(unstated))\n'
               '            continue\n',
        replacement='        except ZeroDivisionError:\n'
                    '            continue\n',
        guard='test_a_master_with_no_record_is_named_and_the_rest_are_stamped',
    ),
    Sabotage(
        suite='python',
        label="the manifest names no sources for a rendered hero",
        path='web/scripts/gen_manifest.py',
        needle='        heroSources=hero_sources(renders, slug) if sizes else [],\n',
        replacement='        heroSources=[],\n',
        guard='test_a_rendered_country_names_what_its_render_read',
    ),
    Sabotage(
        suite='web',
        label="a country page credits a hero on a page with no image",
        path='web/src/lib/countryPages.ts',
        needle='  if (!country.rendered) return [];\n',
        replacement='',
        guard='are none for a country with no image',
    ),
    Sabotage(
        suite='web',
        label="a country page names a dataset twice when its hero and its Focus layer both read it",
        path='web/src/lib/countryPages.ts',
        needle='  return [...new Set([...country.heroSources, ...focus])].map((key) => sourceCard(key));\n',
        replacement='  return [...country.heroSources, ...focus].map((key) => sourceCard(key));\n',
        guard="are what its hero was recorded as rendered from, then the Focus layer's, once each",
    ),
    ...
    Sabotage(
        ...
        path='web/src/lib/archiveIndex.ts',
        needle='      credit: record.credit,\n',
        replacement='      credit: CREDITS.archives[`${body.slug}/relief`].credit,\n',
        guard="credits what its images were rendered from, as the bundle's record states it",
    ),
    Sabotage(
        suite='web',
    ...
    Sabotage(
        suite='python',
        label='a tile window widens on every side for an outline it cut, and cuts the next',
        path='pipeline/look/salt.py',
        needle='        taken = _taking_in(window, boxes, clusters).intersection(grid)',
        replacement='        taken = window',
        guard='test_a_tile_window_that_cuts_an_outline_takes_in_its_cluster_and_nothing_else',
    ),
    Sabotage(
        suite='python',
