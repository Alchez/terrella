...
1. **To run the built site locally the way a real server would**, with production's cache classes
   and compression, offline, and no Cloudflare account.
...
- **Three cache classes.** `/_astro/` is content-hashed, so `immutable` for a year. The asset stores
  and `/caps/` change rarely, so a week plus ETag revalidation. **HTML must be `no-cache`** (meaning
