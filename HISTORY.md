...
- **Instruments wrong before any finding was, and the tally is now the largest of any session.** Four zsh traps: no word-splitting twice (`for x in $VAR`, `set -- $addr`, giving a byte-oracle that compared against `tiles/3 4 3//.webp`), plus **`status` and `PPID` are read-only names**. A watcher that matched a *stale* sentinel because the runner appended to a log that already held one — fixed by counting sentinels, not matching them. A progress readout where `*-3840.webp` silently matched `*-spotlight-3840.webp` and reported **226/203**. And a byte-identity control that could not fail, because the encoder stub wrote the same string the fixture did — caught only by falsifying it. **Every new test in this entry was falsified against the pre-fix state; the one that was not is the one that was vacuous.**
