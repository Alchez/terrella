...
**The build.** `terrain_rgb.py --max-zoom 8` (default also 6 → 8): **41 minutes**, 87,381 tiles, **2.63 GB** — against my projections of 1.5–2.5 h and ~3.3 GB, so I was ~3x over on time and 25% over on size. Cut times z8 **5:29**, z7 1:37, z6 0:27. z8 needs no new elevation intermediate: `MASTER_ZOOM = 8`, so `--max-zoom 8` gives a downsample factor of 1 and encodes `height_3857.tif` in place. The store is now **67 GB**, 60 GB of it the `elev/` chain because `elev_z8.tif` is a 46 GB copy of the master.
