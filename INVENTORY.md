...
| `planet_terrain/` | **11 GB** | Terrain-RGB (Tier 3 displacement), built by `tile/terrain_rgb.py` from `height_3857.tif`. Shipping pyramid is `bathy_s8_webp/` (**2.63 GB, 87,381 tiles, z0–8**), stamped `tiles.done` + `terrain_params.json` so it will not restage, packed to **`terrain.pmtiles` (2.63 GB)** via **`terrain.mbtiles` (2.69 GB)**. The rest are spike A/B builds (`bathy`, `bathy_s2/_s4/_s8`, `clamp`, `bathy_s8_webp_z6`). **The 60 GB `elev/` chain is gone** — reclaimed once proven re-derivable from the master | Mixed — **the spike builds (~3.5 GB) are now reclaimable**: the flags that selected them (`?dem`/`?quant`/`?demfmt`/`?demdepth`) retired with the archive, so nothing can address a build directory any more. `terrain.mbtiles` (2.69 GB) is the bridge format and is reclaimable the moment the archive verifies — it is regenerable from `tiles/` in 12 s. Keep `bathy_s8_webp/` (the pack source) and `terrain.pmtiles` (the deployment artifact). A rebuild re-derives the chain from `height_3857.tif` and no longer copies the master, so it costs ~13 GB transiently, not 60 |
