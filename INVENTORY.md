...
| `tiles_old/` | — | **absent right now**, reclaimed 2026-07-21. `build_tiles` recreates it on the next cut by rotating the then-live pyramid, so the rollback is self-healing. Until that cut, the fallback is `planet_rgb.tif` → a 3:28 re-cut, not an instant `mv` | n/a |
...
| `planet_rgb.tif` + `.done` | 9.9 GB | the **approved** composite (2026-07-21 19:55). `fill_strength=0.15` / `hi=1.12` / `ambient_knee=0.30` / `shadow_warmth=0.55` / `snow_curve=gamma8` / `composite_window_rows=128` / `ICE_LO=0.55`. Matches `shade.KNOBS` defaults exactly, so the freshness guard sees no drift (verified: `planet_rgb fresh -> skip composite`) | Keep — `--tiles` reads it |
