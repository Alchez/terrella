...
- Two gitignored stores hold everything — `data/` (sources + intermediates) and
  ...
  **~476 GB** of a 1.8 TB ext4 root. Sizes approximate.
...
## Work / intermediates — `data/work/` (~319 GB)

| Store | Size | What it is | Reclaim? |
|---|---|---|---|
| `planet_tiles/` | **96 GB** | Tile-pyramid build — itemised below | Mixed — see breakdown |
...
| `planet_terrain/` | **6.1 GB** | Terrain-RGB (Tier 3 displacement), built by `tile/terrain_rgb.py` from `height_3857.tif`. Shipping pyramid is `bathy_s8_webp/` (**2.63 GB, 87,381 tiles, z0–8**), stamped `tiles.done` + `terrain_params.json` so it will not restage; the rest are spike A/B builds (`bathy`, `bathy_s2/_s4/_s8`, `clamp`, `bathy_s8_webp_z6`) still selectable via `?dem`/`?quant`/`?demfmt`/`?demdepth`. **The 60 GB `elev/` chain is gone** — reclaimed once proven re-derivable from the master | Mixed — **the spike builds (~3.5 GB) are now waiting only on flag retirement**: terrain ships on the `full` tier, so the one remaining condition is dropping `?dem`/`?quant`/`?demfmt`/`?demdepth`/`?demsize`, which select directories that will not exist once one archive is packed. `bathy_s8_webp/` is the product until it is packed to PMTiles. A rebuild re-derives the chain from `height_3857.tif` and no longer copies the master, so it costs ~13 GB transiently, not 60 |
...
- **The current reclaim picture:**
  - Nothing is dead right now. The last pass took 35 GB — `tiles_old/` (16 GB, the superseded PNG
    pyramid, its rollback window closed once the WebP archive went live), the `planet.mbtiles`
    bridge (3 GB), `raw/globathy/`'s two zips (16 GB, already extracted and md5-pinned for
    re-download), and the `_ab_shadow2` A/B scratch (1.2 GB, its decision archived in
    HISTORY § cast shadows REJECTED A SECOND TIME).
