...
## Work / intermediates — `data/work/` (~380 GB)
...
| `planet_terrain/` | **67 GB** | Terrain-RGB (Tier 3 displacement), built by `tile/terrain_rgb.py` from `height_3857.tif`. **60 GB of it is the `elev/` chain** — `elev_z8.tif` alone is a 46 GB factor-1 copy of the master, because `--max-zoom 8` downsamples by 1. Shipping pyramid is `bathy_s8_webp/` (**2.63 GB, 87,381 tiles, z0–8**); the rest are spike A/B builds (`bathy`, `bathy_s2/_s4/_s8`, `clamp`, `bathy_s8_webp_z6`) still selectable via `?dem`/`?quant`/`?demfmt`/`?demdepth` | Mixed — **`elev/` is reclaimable** (rebuilt in ~41 min, and z8 is derivable from the master by definition); the spike builds reclaim once Step 4 lands and the flags retire; `bathy_s8_webp/` is the product until it is packed to PMTiles |
