...
**Updated 2026-07-20 (sea-ice feature 2b).** The live `tiles/` (15 GB) is now the **`ICE_LO=0.55` sea-ice pyramid**; `planet_rgb.tif` (11 GB) is its composite. Two rollbacks are on disk, both reclaimable now that 0.55 is confirmed on the globe: **`tiles_old/` (15 GB)** = the `ICE_LO=0.25` pyramid (the cutter's swap), **`tiles_preice/` (14 GB)** = the pre-sea-ice pyramid — keep `tiles_preice` until the south cap lands, then drop. New raster **`seaice_3857.tif` (11 GB)** (row below). A full row-by-row re-measure of the older generations (`planet_rgb_v1`, `tiles_256_gamma8`, the gamma8 baseline) is deferred; free space is now **416 GB** (was 487 at this snapshot's date).
