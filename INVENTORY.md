...
| `mars/` | 11 GB | The MOLA/HRSC blended DEM, one file (`Mars_HRSC_MOLA_BlendDEM_Global_200mp_v2.tif`, 11,384,463,908 B, 106694 × 53347 int16). **The whole of Mars's acquire tier** — there is no per-tile machinery, because there is one download | Mars's planet seam (`fuse/relabel_mars.py`) | Keep — re-downloadable, but it is a ~23 min single-stream fetch and the edition is pinned by size and Last-Modified |
...
| `mars/` | 6.6 GB | **The second body's whole work tree**, nested under its own prefix where Earth's stages sit un-prefixed at the root. `planet/` is 12 KB — a CRS-relabelled VRT over the raw blend plus its seam declaration, no copy of the 11 GB. `planet_tiles/` is the rest: `height_3857.tif` 3.1 GB (32768², the z6 grid), `planet_rgb.tif` 1.1 GB, `hs_3857.tif` 738 MB, `tiles/` 352 MB (5,461 tiles, z0–6), `planet.mbtiles` 345 MB, **`planet.pmtiles` 340 MB** (the deployment artifact) | Mixed — the archive and `tiles/` are the products; `height_3857.tif` restages in 2:12 and the whole pass re-runs in 4:41, so nothing here is expensive to lose |
...
| `_profile_mars_tiles/` | 3 MB | The same two files for the first Mars pass, under its own name because it did not run through `run_pass.sh` — that harness caps at Earth's 16 G (PROCESS § The same pass on Mars) | Keep — the source of PROCESS.md's Mars row |
