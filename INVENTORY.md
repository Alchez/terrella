...
| `mars/` | 4.2 GB | **The second body's whole work tree**, nested under its own prefix where Earth's stages sit un-prefixed at the root. `planet/` is 12 KB: a CRS-relabelled VRT over the raw blend plus its seam declaration, no copy of the 11 GB. `planet_tiles/` holds the two products of the z7 cut, `tiles/` (21,845 tiles, z0–7) and **`planet.pmtiles` 0.963 GB** (the deployment artifact), plus the recipe sidecars and the burnt ice GeoJSONs. **THE RAYTRACED CUT SHRANK THE ARCHIVE AND GREW THE MASTER**, both measured: the archive was 1.40 GB composited, and `planet_rgb.tif` is **10.2 GB** where the composite wrote 4.1 GB to the same path, because raytraced tiles carry less fine detail for WebP to spend bytes on while the master is a lossless three-band render. `planet_composite_ARCHIVE.pmtiles` 1.40 GB is a hand-kept copy of the superseded composited archive, reclaimable the moment its A/B is spent. **Its intermediates are NOT all absent any more**: `height_3857.tif` (11 GB at 65536²) is live again because the raytraced producer cuts every block from it, while `hs_3857.tif` and the rest stay reclaimed, since Cycles computes its own light and builds no hillshade at all | Mixed: the archive and `tiles/` are the products and `tiles/` stays until R2 holds a second copy of the archive. Everything reclaimed rebuilds from raw in ~16:10; the `.done` markers left vouching for absent outputs are safe, since every guard returns "rebuild" for a missing file |
...
| `variants/` | **3.5 GB**, **0.21 GB of it reclaimable** | **the served store**: 1,243 hero WebP (6 rungs, q85 to 1920 / q95 above, **+ a per-country portrait fill rung on 25 of them**) + 1,243 spotlight overlays. The **1,010 `*-border-*.png` rungs (0.21 GB measured)** are still on disk, no longer produced, and drawn by nothing | Keep the WebP: this is what the browser fetches. The border rungs go, here and in R2 |
| `borders/` | 158 MB | per-country transparent border PNGs at full render resolution, the layers the rungs above were downscaled from | **Reclaimable in full.** Its producer is deleted and the detail card that drew the rungs is text only, so nothing builds from these and nothing draws them |
...
| hero variants (gallery srcset + country page) | mean per rung **60 KB / 130 / 222 / 466 / 2,838 / 8,624** (640/960/1280/1920/3840/native WebP) + the portrait fill rung (**2048/2560/3072, 19.0 MB over 25 countries**) | staged behind an IntersectionObserver past the first two cards; srcset picks the rung, and for a portrait country the rung's WIDTH is `rung × aspect` | same | `blender/renders/variants/` |
