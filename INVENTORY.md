...
| `cap/` | 335 MB | Both caps' render intermediates (`pipeline/tile/cap_render.py`): AEQD warps + `cap_{north,south}.tif` + **the cap freshness sidecars `cap_{north,south}_params.json`** (2026-07-22 — what `cap_is_fresh` compares against). Served outputs: `web/public/dev-assets/cap_north.png` (13.9 MB) + `cap_south.png` (4.5 MB) | Reclaimable — regenerated by a cap render (deleting the sidecars merely forces one) |
