...
| `mars/` | 12 GB | Two whole-planet downloads, no per-tile machinery. The MOLA/HRSC blended DEM (`Mars_HRSC_MOLA_BlendDEM_Global_200mp_v2.tif`, 11,384,463,908 B, 106694 × 53347 int16) is the heightfield. `Mars_Viking_ClrMosaic_global_925m.tif` (797,888,177 B, 23059 × 11530 RGB) is **evidence, not an input** — no pipeline stage reads it; it is what the land ramp's hue was measured against, kept so the derivation can be re-run | the DEM feeds Mars's planet seam (`fuse/relabel_mars.py`); the mosaic feeds nothing | Keep the DEM — re-downloadable, but a ~23 min single-stream fetch with its edition pinned by size and Last-Modified. The mosaic is **reclaimable**: nothing breaks without it, and its findings are recorded at `palette.MARS_LAND_STOPS` — re-fetching costs ~90 s |
...
| `mars/` | 7.4 GB | **The second body's whole work tree**, nested under its own prefix where Earth's stages sit un-prefixed at the root. `planet/` is 12 KB — a CRS-relabelled VRT over the raw blend plus its seam declaration, no copy of the 11 GB. `planet_tiles/` is the rest: `height_3857.tif` 3.1 GB (32768², the z6 grid), `planet_rgb.tif` 1.2 GB, `hs_3857.tif` 839 MB, `tiles/` 406 MB (5,461 tiles, z0–6), `planet.mbtiles` 399 MB, **`planet.pmtiles` 395 MB** (the deployment artifact), plus `tiles_old/` 429 MB — the previous cut, which the tile stage keeps rather than deletes so a bad cut can be compared against the one it replaced | Mixed — the archive and `tiles/` are the products; `height_3857.tif` restages in 2:12 and the whole pass re-runs in 4:41, so nothing here is expensive to lose. `tiles_old/` is reclaimable the moment a cut is accepted |
| `mars/cap/` | 789 MB | **Orphaned.** Two 8192² AEQD discs rendered while measuring whether a Mars cap pass would succeed — it does, from the heightfield alone. `MARS.renders_polar_caps` is now `False`, so no stage will ever refresh or read them | Reclaimable, and nothing depends on it |
