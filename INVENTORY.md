...
- Two gitignored stores hold everything — `data/` (sources + intermediates) and
  ...
  **~422 GB** of a 1.8 TB ext4 root. Sizes approximate.

## Raw sources — `data/raw/` (~693 GB)
...
## Work / intermediates — `data/work/` (~354 GB)

| Store | Size | What it is | Reclaim? |
|---|---|---|---|
| `planet_tiles/` | **137 GB** | Tile-pyramid build — itemised below | Mixed — see breakdown |
...
- **The current reclaim picture:**
  - Nothing is dead right now — the last pass took 52 GB (ratified rollback trees, the mbtiles
    bridge, three finished experiment dirs) → HISTORY § the reclaim log moves out of INVENTORY.
  ...
  - `raw/globathy/` (16 GB zips) — extracted already; re-downloadable against a pinned md5.
    The cheapest large reclaim if space gets tight, at the cost of a re-download.
