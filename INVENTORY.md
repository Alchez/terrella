...
| `tiles_old/` | 14 GB | **rolled forward 2026-07-18 15:04:** now the **128 dark-cap** pyramid (`CAP_RGB=(67,118,132)`, the pre-pale-C look), 62,177 tiles — the pale-C `--tiles` re-cut renamed the previous live `tiles` over it. **Rollback for the pale-C cap** — `mv tiles tiles_paleC && mv tiles_old tiles`. (The 256 pyramid it used to hold was renamed to `tiles_256_gamma8` first, or the rotation would have `rm`'d it.) | Keep, but the whole flat cap (dark AND pale) is **superseded** by the polar-stereo custom-layer cap → HISTORY § the polar cap: flat fails |
| `tiles/` | 14 GB | **live** z0–8 512px pyramid, 62,177 tiles — served via `TILES_STORE`. Cut 2026-07-18 15:04 from the **128 pale-C composite** (`CAP_RGB=(216,226,233)`). **Interim:** the pale-C flat cap reads as a white plug and is superseded by the coming polar-stereo custom-layer cap; leave-vs-revert-to-dark (`tiles_old`) is Rohan's low-stakes call | Keep (live) until the cap work re-cuts |
| `planet_rgb.tif` + `.done` | 11 GB | **the 128 pale-C composite (2026-07-18 14:54)** — source of the live `tiles/`. Same 128/N4 threaded pass + look knobs (`fill_strength=0.15`, `hi=1.12`, `snow_curve=gamma8`, `composite_window_rows=128`), only `CAP_RGB → (216,226,233)`. The prior dark-cap 128 raster was overwritten in place (reproducible: revert `CAP_RGB` + recompose). 256 preserved as `planet_rgb_gamma8_baseline.tif` | Keep — `--tiles` reads it (until the polar-stereo cap supersedes the baked cap) |
