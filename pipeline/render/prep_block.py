...
def mid_latitude_deg(window: Window, body: bodies.Body) -> float:
    ...
    One owner, which is the whole reason it is a function: `ground_width_m` multiplies by this
    cosine and `row_scale` divides by it, so the two must agree on the same row or the plane comes
    out uniformly exaggerated at the wrong value. That failure has no symptom, every seam
    measurement reading clean while the whole planet is scaled wrong, and two spellings of "the
    middle" is all it would take.
    """
...
def ground_width_m(window: Window, body: bodies.Body) -> float:
    ...
    toward the equator by `cos(latitude)`, and a Mercator unit is not a ground metre on a body whose
    own radius differs from the projection sphere's, which is exactly 1.0 on Earth.
    `bodies.ground_metres_per_mercator_unit` owns that ratio, and a copy that dropped it would
    undersize the great majority of Martian blocks while being perfect on the only planet anyone
    tests against.

    One width is all a plane can have, which is not the same as one width being right: the plane is
    a single rectangle so its Blender-unit scale has to be a single number, and Mercator's ground
    scale is not. `row_scale` carries the difference on the displacement rather than here.
    """
...
def row_scale(window: Window, body: bodies.Body) -> NDArray[np.float64]:
    ...
    The defect it removes: `displacement_scale` converts elevation metres into Blender units through
    `ground_width_m`, which is one number taken at the plane's centre row. Mercator's ground metres
    per pixel vary continuously with latitude, so that number is right on exactly one row, and
    neighbouring block rows disagree across their shared edge by the ratio of their two centre
    ...
    and puts shadow reach on the occluder's latitude; the rule beside this file owns what that means
    for the blocks around it.

    Clipped at Mercator's limit because `block_plan.row_latitude_deg` clips there, and a plane may
    extend past the grid on the pole side. Those rows carry the clamped edge elevation rather than a
    fill, so the value is felt and has to stay finite, and this uses the same spelling of "the
...
def _read_cyclic(dataset: Any, window: Window) -> np.ndarray:
    ...
    The two axes are not symmetric, which is the same split `Block.plane_window` makes: a Mercator
    planet joins itself at the antimeridian and does not join itself at the poles. Both halves are
    here because a plane may overhang either edge and neither overhang may invent geometry.

    Neither axis may be filled, because the datum is not a neutral value. On the columns a fill puts
    a sea-level plateau where the far side of the antimeridian belongs, reads as land on the ocean
    mask, and the north-west sun casts that wall east across block column 0. On the rows it does the
    same thing standing up: both grids end well below zero, so filling with 0 raises a wall of real
    elevation proud of the ground it replaces, which at these exaggerations models as kilometres of
    cliff along the north edge of every row-0 plane. Clamping repeats the edge row, the one choice
    that adds nothing, and it is what `plane_window` documents.
...
def write_mask(out: Path, array: np.ndarray) -> None:
    ...
    rig, so the depth below is one law with two readers and this is its owner.
    ...
    The depth is a geometry constraint and not a colour one, which is why 8 bits was not enough. The
    rig wires the sea-ice alpha to two arms, an ice-white colour mix and the flatten that pulls
    displacement toward sea level, so a quantised alpha terraces the sea floor rather than merely
    banding its colour and each level boundary becomes a riser of `depth * quantum * exaggeration`
    metres. Over abyssal water at 8 bits that is a step taller than the ground one pixel covers, so
    it is a slope past 45 degrees against a 45-degree sun and every boundary self-shadows into a
    line. `test_a_quantised_alpha_does_not_terrace_the_sea_floor_past_one_ground_pixel` is the
    bound, stated as that physical law rather than as a bit count so it cannot rot behind a retune.
    """
...
def _write_rowscale(out: Path, window: Window, body: bodies.Body) -> None:
    ...
    row by construction, so a plane-sized field would carry no extra information, and Blender
    expands a float image to RGBA float, which on the widest plane is about a gigabyte on top of an
    already heavy render.

    It calls `row_scale` and must go on doing so. A copy of the law here would be inert under every
    mutation `scripts/sabotage.py` makes, so its cases would report CAUGHT while the raster the rig
    actually samples stayed unmutated, which is a harness lying in the direction that reads as safe.
...
def one_colour(value: Any, what: str) -> render_seam.RGB8:
    ...
    A paint may legitimately vary and a rig shader socket cannot, so the reduction has to be a check
    rather than an average. Mars answers `(3, H, 1)` because its two poles are different colours;
    that array is constant within any block, splitting at the equator where Mars carries no ice
    within 76 degrees. A block straddling the split would be a real conflict with no correct single
    answer, and averaging two measured whites into a third that neither pole has would render it as
...
def merged_paint(paints: dict[str, tuple[Any, Any]], members: "tuple[layers.Layer, ...]",
    ...
    The fold is a maximum over alphas and carries no colour, so several layers land in one image and
    the rig gets one socket for it. Earth's perennial ice and glaciers both answer `_earth_paint`
...
def build(body: bodies.Body, window: Window, outdir: Path, *, work: Path) -> list[str]:
    ...
    The paints are declared here and travel with the masks they belong to: `gather` resolves each
    layer's colour from this body's own registry for this window, `_earth_paint` for Earth and
    `_mars_ice_paint` per pole for Mars, and dropping them leaves the rig holding one module-level
    white, which is Earth's, so every body renders in it and Earth is indistinguishable from
    correct. `render_seam.PAINTED_IMAGES` holds why the answer has to travel as data.

    `work` is the caller's and is never re-derived here, which is what makes `block_render --work` a
    seam rather than half of one; the rule beside this file owns the rest. Keyword-only because it
    and `outdir` are both directories, and a swapped pair would read the render directory as a
    planet and write the block into the store.
    """
...
def write_frame(body: bodies.Body, block: Block, outdir: Path) -> dict[str, Any]:
    ...
    Where the three widths meet, each reaching the rig as a different number. The heightfield is the
    plane, so `width_px`/`height_px` and the ground extent are the plane's; the render resolution is
    the traced edge, a block rendering 1:1 with the grid where only that rectangle is photographed;
    and `camera_fraction` is the ratio between them, which is what narrows the camera inside its own
    plane. A hero has no such distinction, which is why the fraction defaults to the overshoot.
    """
...
def write_recipe(body: bodies.Body, window: Window, outdir: Path, written: list[str]) -> None:
    ...
    where a commit id names the whole tree and moves on every checkout that touches anything at all.

    It is not what makes a block restage. A pass deletes this directory and skips blocks by marker
    existence, so nothing compares two generations of it and `block_render.params` has the teeth.
...
def cut(body: bodies.Body, block: Block, outdir: Path, *, work: Path) -> dict[str, Any]:
    ...
    It takes a `Block` rather than four numbers, so that the window arithmetic has one owner: two
    copies of it were tied together by nothing, and changing one moved no test.
    """
    window = block.plane_window
