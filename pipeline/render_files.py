...
At the top level because code beyond the rig reads these spellings: `render/` fills a directory,
`tile/` raytraces a block out of one, `compose/` reads the heightfield and ocean mask, and `batch`
drives the run. `render/render_seam.py` is the other half, the declaration of what a stage actually emitted,
...
#: The four a prep may legitimately not write, being measurements of the region rather than of
#: the planet: a block with no snow in it, no lake bed in it, no sea ice on its ocean, or no salt
#: flat. The ice image is a continuous 0..1 alpha, already confined to ocean pixels by the prep
#: that cut it.
SNOWMASK = "snowmask.png"
LAKEDEPTH = "lakedepth.tif"
SEAICE = "seaice.png"
SALTMASK = "saltmask.png"
