...
- **Two MapLibre APIs take a screen point and disagree about what one is.** `transform.screenPointToLocation` accepts `{x, y}`; `map.queryRenderedFeatures` does **not** — handed one it returns **zero features**, with no throw and no warning, which reads exactly like a map with nothing rendered. It wants `[x, y]` or a real `Point`. Check the point type per call site; never generalise from the forgiving one.
...
When probing the live map's own DEM — the only oracle that separates "loaded but not rendered" from "never loaded":

- The decoded tiles are at **`map.terrain.tileManager.tileManager._inViewTiles._tiles`** — the doubled `tileManager` is real. `map.terrain.sourceCache` and `_sourceTileCache` are dead ends.
...
- **The idle spin moves the camera under a probe** — `await map.once('idle')` can hang forever at overview zooms. Pin with `jumpTo` immediately before reading, and re-read `map.getZoom()` beside the result.
