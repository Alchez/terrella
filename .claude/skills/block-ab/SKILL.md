...
- **A cap has no `--work`, so redirect in-process.** Every writer calls `cap_render.cap_work_dir` at call time, so replacing that function in a scratch script sends the AEQD warps, the render directory and the frames to scratch while the inputs still come from the store. Create `cap_raytrace.frames_dir(grid)` first: `render` makes it and `render_frame` assumes it. Snapshot the store's cap folder before and after, as above.
  - **`frames_dir` and `frame_path` follow the redirect too**, so a shipped frame read for comparison has to be built from the unredirected work dir, or the arm silently compares against a path holding nothing.
- **A reused render directory pins every painted colour to the day it was cut**, because `render_seam.paint_for` reads the declaration beside the images and never the palette. So an arm that copies a prep measures rig changes only, and a moved `palette` white reaches nothing: re-cut the prep to see one. The tell is a recipe diff naming colours the frames cannot be showing.
- **One production-size frame a pole is the cheap sample, and a smaller disc is not.** `frame_plan` says which bearings a quadrant needs, and a pixel takes one frame whole where its bracket starts at that bearing, so that frame IS the disc there. Rendering the disc at a reduced `px` instead changes the ground each pixel covers, which is the wrong sample for anything acting at pixel scale, bump above all.
...
- **A surface colour is read at build time through a module-level seam**, so replace it before `main()`: `scene_build.declared_albedo` for snow, salt and sea ice, `palette.LOOK_BY_BODY[body]` for the land and sea ramps and the fill's temperature, and `scene_build.RIG` through `dataclasses.replace` for lakes and river water. Each is looked up when called, which is what lets the replacement reach the build.
- **Every surface but the land is then multiplied by `fill_compensation`**, so a colour replaced at those seams is the authored one and renders as that colour on flat sunlit ground, not as the albedo. An arm that changes the fill moves the compensation with it; to see a fill uncompensated, replace `scene_build.fill_compensation` too.
- **A camera narrowed onto a small window does not render that window as the block does.** Cycles meshes the window differently: on the Alps block at one micropolygon a pixel the window came out 1.1 DN darker from edge to centre, with bump barely moved. Such arms compare only with each other. To reproduce the block's own pixels, keep its camera and render a crop border, which costs the whole block's memory.
