...
- **What it costs**: the heroes have to re-fuse for the re-render anyway, so their half is the fix itself. The planet half is a re-fuse of every cell, every warp after it and a whole Earth pass for no visible change, so it rides the next Earth pass owed for another reason. HISTORY, *what the tile passes share with the hero queue*.
...
Every part of it is a HERO deficiency, and none of it changes a raytraced tile.
...
- **The next action** is a measurement: how much white sits on level ground that neither source touches, Sirjan's kind, which sizes what the union leaves as it is.
- **On the shipped Earth tiles the salt reads as white paint**, the maintainer's read: one flat colour on ground the elevation data flattened, with a 1 to 2 px edge. A look round on the edge and the tone is the maintainer's call, and variation inside a flat would need a new source or a new rule. HISTORY, *the salt reads as paint*.
...
- **The block-row seam's fix rode the Earth pass**, whose recipe records `row_edge_mode` `"edge"`; the join on the shipped tiles is unmeasured.
...
- **`SITE-4`'s publish owes the maintainer's look at what the build left open**: "Download:" alone on its line at 320 px, the Country maps card alone on a second row at 1440, the SHA-256 on that card, and the words of the bundle's `README.txt`. Zooming a country image in Zen is the only check that a full-size file marked to download still paints in Firefox, which was tested in Chromium alone. `docs/pipeline.md` § *Publishing heroes (Earth only)* runs the publish.
- **Once the bundle is public, a re-render changes its bytes, and nothing stops the rebuilt one going up over `earth/country-maps-webp-v1.zip`.** It first goes up holding the July heroes, so the re-render's is the first rebuild this catches. The bucket is additive by decision, so it goes up as v2 with v1 kept, which needs `downloads.BUNDLE_KEY` bumped and a superseded row the Archives page can list, `ARCHIVED` holding tile cuts alone. The deploy preflight refuses until R2 holds what `web/src/data/downloads.json` records, and an upload over v1 satisfies it.
