...
- **Three costs are MapLibre's, and Cesium removes one.** The cap is bespoke because sources are Mercator only; the terrain seam is structural because one mesh is shared across tiles; terrain VRAM peaks with the renderable-tile count, which nothing in MapLibre bounds. Cesium's quantized-mesh removes the seam and nothing else here.
...
- **A re-render changes the public bundle's bytes, and nothing stops the rebuilt one going up over `earth/country-maps-webp-v1.zip`**, which holds the July heroes. The bucket is additive by decision, so it goes up as v2 with v1 kept, which needs `downloads.BUNDLE_KEY` bumped and a superseded row the Archives page can list, `ARCHIVED` holding tile cuts alone. The deploy preflight refuses until R2 holds what `web/src/data/downloads.json` records, and an upload over v1 satisfies it.
