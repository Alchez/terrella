...
- **`data/work/<country>/render/frame.json`** — per-country derived numbers (displacement_scale,
  ortho_scale, resolution), plus the `body` and `exaggeration` they were derived at. Written once by
  `render_prep.py`, never overwritten, so a hand-edited value is a durable per-country override.
  `FRAME_MARGIN` and `HERO_LONG_EDGE` live in `render_prep.py`; the exaggeration comes from
  `Body.exaggeration`. Delete a frame.json to re-derive, hand-edit to pin.
...
| terrain polar encode (pipeline/tile/terrain_rgb.py) | **There is no polar feather, and adding one back is a REJECTED shape rather than an open lever.** A ramp flattening the encode toward datum over 78°–85° did ship, written when the cap could not displace at all; it reopened its own seam the moment `polarCaps.vertexSrc` began lifting cap vertices, leaving the tiles flat against a cap holding true elevation. The cure was deleting the ramp, not re-banding it, so the argument to refuse is any of the form *"the poles need special treatment in the encode"* — nothing here may depend on where a pixel is, because `cap_render.write_cap_elevation` calls the same function for an azimuthal grid with no rows | n/a — the lever is gone |
