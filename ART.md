...
- **`data/work/<country>/render/frame.json`** — per-country derived numbers (displacement_scale,
  ...
  value is a durable per-country override. The *formulas* (EXAGGERATION, FRAME_MARGIN,
  HERO_LONG_EDGE) live in `render_prep.py`; delete a frame.json to re-derive, hand-edit to pin.
...
| Lever | Value | Section |
|---|---|---|
| `EXAGGERATION` (render_prep) | 15.0 — **copy-paired with tile `EXAG` (shade_planet.py): audit-matched, NOT import-shared** — the one surviving copy pair | § Vertical exaggeration |
...
| tile `EXAG` (shade_planet.py) | 15 — the hero copy-pair, see above | hillshade | § Vertical exaggeration |
...
- **Adjust globally:** `EXAGGERATION` in render_prep.py, then regenerate frame.json per country
...
|---|---|---|
| `EXAG` 15 (shade_planet.py) | `render_prep.EXAGGERATION` 15.0 | same number, separate constants — the one surviving copy pair |
...
- **Adjust:** `--strength` (0.38), `--threshold` (0.45), `--svf-long` (compute res). Tile-side
  the analogues are `svf_strength` / `svf_threshold` in KNOBS.
