...
| Worker deploy + first TLS | `npx wrangler deploy` in `web/worker` | deploy seconds; **certificate ~7 min** | A **two-level** hostname is not covered by Universal SSL (`*.zone` is one level): expect `TLS alert handshake failure` while DNS already resolves, until the auto-ordered advanced cert issues. A one-level hostname is live immediately |
