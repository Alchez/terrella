...
| PMTiles packaging — terrain, Mars | same with `--body mars --out …/planet_terrain/terrain.mbtiles` | dir→MBTiles **2 s** (21,845 tiles, 0.78 GB); convert **1.4 s** → **0.766 GB** `terrain.pmtiles` | **Dedupe all but vanishes on a body with no sea: 21,840 unique of 21,845, 0.02%**, against Earth's 1.4% — Earth's repeats are flat ocean, and Mars has none, so only the feathered polar tiles collapse. **Index is 50,702 B**, a quarter of Earth's, well under `INDEX_PREFETCH_BYTES`. The flip-sensitive pair to byte-compare here is `z7/64/0` against `z7/64/127` — same column, opposite poles, and both tiny (100 B / 84 B) because the feather flattens them, so a swap would not show up in a size check alone |
