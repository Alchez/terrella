...
# `hillshade_for_light` stood here and went with `shade.KNOBS`. It inverted the compositor's
# softplus ambient floor so a test could ask for a pixel at a known light, and every caller was a
# composite look test. Nothing in the suite called it by the time the knobs were pruned.
