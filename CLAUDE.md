...
- Snow and glaciers differ between the two surfaces by decision rather than drift. Tiles take NSIDC-0791 persistence plus every RGI 7.0 region, which `download_rgi` enforces rather than assumes, with a latitude-ramped soft alpha; Antarctica is painted white outright, because persistence saturates there and still leaves clustered fill that RGI's peripheral region does not reach, whitening the sub-Antarctic islands instead where persistence has no observation at all. Heroes take ESA WorldCover class 70, which the tiles replaced because permanent-ice-only left mid- and high-latitude ranges bare.
