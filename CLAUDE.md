...
borders, minimal typography. The aesthetic decisions live in docs/ART.md.
...
- **Two rasters are fused and no others:** Copernicus DEM GLO-30 for land and GEBCO for bathymetry, into one seamless heightfield. The bathymetry is part of the signature look rather than an option. Everything else is warped onto the render grid by the planet warp stage and never enters the fusion master, which is why a finer re-fuse would not have to redo any of it.
- Lake beds are tint-only. A carved bed at the baked exaggeration makes a crater of every lake and kills the flat plate that catches the surrounding shadows. Modelling every lake alike is deliberate: restricting to surveyed ones renders survey funding as geology, the discontinuity falling on the US/Canada border.
- Snow and glaciers are one law on both surfaces: NSIDC-0791 persistence plus every RGI 7.0 region, which `download_rgi` enforces rather than assumes, with a latitude-ramped soft alpha. The hero snow stage folds the tiles' own producers on a country's grid, so the grid is the only difference. Antarctica is painted white outright, because persistence saturates there and still leaves clustered fill that RGI's peripheral region does not reach, whitening the sub-Antarctic islands instead where persistence has no observation at all.
- Salt flats take a tone of their own on both surfaces, where Natural Earth's playas or GWL_FCS30's saline class put them: NSIDC-0791 reads their crust as snow and the water mask reads it as lake, so the salt takes its share of the finished white and is painted last, over the lake. `look/salt.py` owns which ground is salt, and `acquire/earth/extract_gwl.py` which saline cells it reads: only those ESA WorldCover calls bare, snow or water, since the class takes in salty ground under plants.
- Borders are never baked into raster tiles or heroes. The globe draws them as a white vector overlay, and a hero carries none: its page's Focus overlay strokes the country's own boundary, composited over the image rather than rendered in the Blender scene. Worldview is Natural Earth default (de-facto) site-wide, disputed segments dashed, noted on the About page.
- Mars has no sea, no borders and no snow dataset, and the share-alike half of its blended DEM's licence is why the whole site's output is share-alike rather than permissive. `ATTRIBUTIONS.md` owns the exact licence, and the About page publishes it.

## Rendering decisions

- **Heroes:** headless Blender Cycles (bpy) on whatever backend `scene_build.GPU_BACKENDS` finds, plus OpenImageDenoise, but **CPU denoise for 8K**, or render and denoise contend for one card's memory and the driver faults. The rig raises rather than falling back to CPU, which would be correct, slower and identical on disk.
...
- Vertical exaggeration belongs to the body, and `Body.baked_exaggeration` is where the number lives, which is why it is not spelled here. Every path that draws more than one body reads it, the render seam included since it is the block prep's as well as the hero's; `palette.EXAGGERATION` is the authored constant, which no production path reads and a test pins Earth's field equal to. Unpinned, the tiles drift away from the heroes they must match. The name says which of the two the pipeline owns: the browser's `meshExaggeration` displaces a live mesh and never reaches a pixel this one is baked into.
...
- **A numpy cast-shadow term is a REJECTED idea, not an open one, and it no longer has a caller to be reopened into.** Every body's tiles are raytraced and carry real cast shadows from Cycles. The rejected mechanism was `cast_shadow.shadow_mask` attenuating a composited main sun: that scales light amplitude, and fine detail amplitude falls with it, so such a shadow erases the modelling it carries. It was turned down twice, the second time on the mechanism rather than the value. `shadow_mask` and `sun_offsets` are now unreachable from production, their only caller having gone with the compositor; `cast_shadow.shadow_reach_px` is live and unrelated, four lines of arithmetic that size every block's context in `block_plan`.
- Tile depth belongs to the body too, and `Body.tile_max_zoom` owns it for the same reason. **Earth's ceiling is LOCKED**, since the deeper pyramids are parked in FUTURE and blocked on disk, needing a planet re-fuse at a finer grid rather than a tiling flag. FUTURE's entry prices both.
- A tile asset and the size it is DECLARED at are different numbers, and the two pyramids declare differently: relief's centres the scheme on DPR 2, terrain's is smaller and is what makes terrain draw twice the tiles per view. `reliefSources.DECLARED_TILE_SIZE` and `terrainSource.TERRAIN_TILE_SIZE` own the values, which is why neither is spelled here, and `web/DEPLOY.md` prices the consequence. → FUTURE § raster tile resolution vs device pixel ratio
...
- Dev/render box: a dual-boot desktop. **All work happens in the Ubuntu boot**: never suggest Windows paths, WSL, or PowerShell.
...
- Keep intermediates out of backups. docs/INVENTORY.md is the storage map: current sizes, what each store feeds, and which are reclaimable.
...
- **Every prose change goes through the `prose-pass` skill before it is called done**: comments, docstrings, messages and docs alike. Its four checks are the start, and reading each added line against the code is the pass.
...
- **A comment explains its own subject and claims nothing that can rot behind its back.** Trigger: could this sentence go false without anyone touching this function? Counts, measurements taken elsewhere and system-wide properties all can, and belong in a test, in docs/PROCESS.md, or nowhere. Keep the concept, the context needed to read the code, and the anti-redo guard.
- **No figure describes the maintainer's machine**, in a doc, a comment or a test: not a GPU or its memory, RAM, cores, a disk or its free space. `docs/PROCESS.md` names the reference machine once, at its top, so its runtimes have a baseline, and everywhere else says "the reference machine". Measurements of the work are not machine figures. A limit a visitor's box may exceed is a default with an announced override, never a number read off the host.
...
- The other docs, so facts are looked up rather than re-guessed: **docs/PROCESS.md** measured runtimes (the authority, read it before estimating), **docs/INVENTORY.md** the storage map, **docs/ART.md** the aesthetic decisions, **FUTURE.md** the v2 parking lot (check it before designing a "new" feature), **docs/pipeline.md** how to run a pass, **docs/adding-a-body.md** what a new planet must answer for, **docs/pipeline-layout.md** where a module goes, **docs/*.mmd** the pipeline diagrams. A body seam is read from `pipeline/bodies.py` and its web twin, which own every per-body fact between them.

Working in Blender, the version's shader gotchas and the crash recipe are their own skill
