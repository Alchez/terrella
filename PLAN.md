...
- [!] **THE ONLY REMAINING STEP: cut the tiles.** `python -m pipeline.tile.shade_planet --tiles` (gdaladdo + `gdal raster tile` z0-8, ~62 k tiles; swaps `tiles/`, keeps `tiles_old/` as rollback). **The live globe still serves the PRE-Caspian, PRE-GLOBathy pyramid** from `planet_rgb_v1.tif` — none of this work is visible until this runs. Everything upstream is fresh and verified, so it is tiling only. Gated on Rohan judging the mosaic. **Cost:** ~36 GB disk peak (486 GB free — non-issue) and **all 16 cores** during tiling (`gdal raster tile` defaults `-j ALL_CPUS`), so not alongside other work; wall time has never been measured in isolation — time it on the next run. The 10× writer flag does **not** pay here (planet_rgb fresh → composite skipped). **gdaladdo DELETED** — `gdal raster tile` never reads source overviews (byte-identical, proven), so the step was ~3 min + ~4 GB of nothing; `--webviewer=none` and a `--overview-resampling=cubic` pin landed with it, both byte-safe. → HISTORY § the gdaladdo step DELETED
