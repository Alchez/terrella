...
- [~] **Polar cap via a MapLibre custom layer — NORTH cap DONE (sea ice + bathymetry, behind `?polarspike`); SOUTH cap (Antarctica) is Phase 2.** Mercator can't bake the poles (ends ~85°N), so each cap is a source-shaded polar raster (reaches 90°) drawn over the pole by a `CustomLayerInterface` layer, feathered into the tiles at ~85°. **Pin `maplibre-gl` 5.24.0.** → HISTORY § the polar cap: flat fails · § the cap's seam-match · § 2026-07-20 sea ice
  ...
  - [~] Productionize — `cap_render.py` now lives in `pipeline/tile/` (done); still open: **south cap (EPSG:3031, Antarctica — Phase 2)**, downsize the 15 MB cap PNG for prod, drop the `?polarspike` flag, fold into PMTiles
  - [ ] Cap layer should restore GL state each draw (leaks `BLEND` + vertex-attrib arrays — a latent custom-layer-contract bug; NOT the border artifact, which was the country-highlight below) — **fold into the Phase 2 `polarCapSpike.ts` N/S-factory refactor**
...
- [~] About page: data credits (Copernicus, GEBCO, Natural Earth, ESA WorldCover — exact CC-BY string in the locked-constants Snow entry; **OSI SAF sea ice + the 1991–2020 reference-period note added 2026-07-20**), technique notes
...
- **Mercator can't reach the poles → a polar-stereographic custom-layer cap** (data ends 85.05°N; `1/cos φ` sends the pole to ∞). Flat fill fails (dark = hole, pale = plug); the cap is a source-shaded polar raster drawn over the pole via a MapLibre custom layer, sea ice over bathymetry. Live tiles still carry the interim pale-C plug under the cap. → HISTORY § the polar cap: flat fails · § the cap's seam-match
