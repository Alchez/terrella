...
- [~] **Polar cap via a MapLibre custom layer — bathymetry base DONE (behind `?polarspike`); sea ice next.** Mercator can't bake the poles (ends ~85°N), so each cap is a source-shaded polar raster (reaches 90°) drawn over the pole by a `CustomLayerInterface` layer, feathered into the tiles at ~85°. **Pin `maplibre-gl` 5.24.0.** → HISTORY § the polar cap: flat fails · § the cap's seam-match
  ...
  - [ ] 2b — sea ice draped over the bathymetry (data-driven edge; ties to the Antarctica south cap)
  - [ ] Productionize — `scratchpad/cap_render.py` → `pipeline/`; drop the `?polarspike` flag; south cap (EPSG:3031); fold into PMTiles
  - [ ] Cap layer should restore GL state each draw (leaks `BLEND` + vertex-attrib arrays — a latent custom-layer-contract bug; NOT the border artifact, which was the country-highlight below)
