...
- [ ] **Deferred gaps, now accept-or-fill decisions in their own right** (they outlived the judgment that surfaced them): **Antarctica** (flat cap below −60°S — `fuse_planet.py --skip-south -60`; GEBCO is ice-*surface*, so it needs its own GLO-30 pass, never a bathymetry clamp) and **Greenland interior** (flat white where snow persistence = 1 over smooth ice). Neither blocks PMTiles. → HISTORY § THE TILE CUT LANDED
...
5. **Parallelise the composite with `ThreadPoolExecutor`, ~4 threads** — *not* processes. Measured 2026-07-16 on real windows through `shade.composite`: numpy releases the GIL, so threads scale **1.80× @2 / 2.83× @4 / 3.57× @8**, using 3.54 cores at 4. Efficiency falls 90%→45% as memory bandwidth saturates, so **~3× is the ceiling and 4 threads is the knee** (54→~19 min). Threads need no IPC, no pickling and no per-worker GDAL handles — a `ProcessPoolExecutor` design was drafted and killed by this measurement. Gated on #4 (the fixed-path snow temps are not concurrency-safe). Read/write stay on the main thread; rasterio datasets are not thread-safe. **The ~3× is a property of the WINDOW SHAPE, not of numpy or dask** (noted 2026-07-17): full width ⇒ every 3-channel array is 402 MB against ~32 MB of L3, so all ~30 ops are DRAM round-trips and threads cannot add bandwidth — which is exactly what "efficiency falls 90%→45%" is measuring. **Unexplored:** narrow, cache-sized windows (4096 px ⇒ ~12 MB arrays) might beat 3× outright, and would also make RAM spare enough to buy more threads; needs #4 first (32× more windows ⇒ 32× the forks). **Untested — a hypothesis, not a plan; measure before acting.** Note the direction: smaller, never bigger — enlarging windows for speed is the 2026-07-14 OOM.
